...
namespace Excel2Charp
{
    public class ExcelToScriptableObject : EditorWindow
    {
        ...
        [MenuItem("Tools/ConfigTools/Excel2ScriptableObject")]
        ...
        static bool Process(ExcelToScriptableObjectSetting excel, bool generateCode)
        {
            ...
            if (excel.generate_get_method_if_possible)
            {
                ...
                if (fieldTypes[0] == eFieldTypes.Int)
                {
                    ...
                    {
                        if (EditorUtility.DisplayCancelableProgressBar($"Process {excel.excel_name}", string.Format("Checking IDs or Keys... {0} / {1}", i - 1, imax - 2), (float)(i - 1) / (imax - 2)))
                        {
                        ...
                        {
                            err = "config: " + excel.excel_name + "  --  " + string.Format("Fail to parse '{0}' in line {1} because it seems not to be a 'int'", idStr, i);
                            break;
                        }
                        List<int> idList;
                        if (ids.TryGetValue(id, out idList))
                        {
                            ...
                            {
                                err = "config: " + excel.excel_name + "  --  " + string.Format("ID or key:{0} appeared more once...", id);
                                break;
                            }
                        }
                        else
                ...
                else if (fieldTypes[0] == eFieldTypes.String)
                {
                    ...
                    for (int i = global_configs.data_from_row, imax = table.Rows.Count; i < imax; i++)
                    {
                        if (EditorUtility.DisplayCancelableProgressBar($"Process {excel.excel_name}", string.Format("Checking IDs or Keys... {0} / {1}", i - 1, imax - 2), (i - 1f) / (imax - 2f)))
                        {
                            EditorUtility.ClearProgressBar();
                            return false;
                        }
                        ...
                        if (keys.TryGetValue(key, out idList))
                        {
                            ...
                            {
                                err = "config: " + excel.excel_name + "  --  " + string.Format("ID or key:{0} appeared more once...", key);
                                break;
                            }
                        }
                        else
            ...
            else
            {
                ...
                {
                    if (EditorUtility.DisplayCancelableProgressBar($"Process {excel.excel_name}", string.Format("Serializing datas... {0} / {1}", i, imax), i / (float)imax))
                    {
                        EditorUtility.ClearProgressBar();
                        return false;
                    }
                ...
                if (excel.use_hash_string && hashStrings.Count > 0)
                {
                    ...
                    {
                        if (EditorUtility.DisplayCancelableProgressBar($"Process {excel.excel_name}", string.Format("Writing hash strings ... {0} / {1}",
        ...
        enum eFieldTypes
        {
            Unknown, Int, Ints, Long, Float, Floats, Vector2, Vector3, Vector4, Rect, Color, String, Strings, Skip
        }
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Copy Excel", GUILayout.Width(100f), GUILayout.Height(30f)))
            {
                CopyExcel2Project.CopyExcelToProject();
            }
