...
namespace UGame_Remove_Tmp
{
    public class AudioPlayerBase
    {
        ...
        {
            AudioClip red = JResource.LoadRes<AudioClip>($"AudioSource/{name}", JResource.MatchMode.Other);
            ...
            Log.PrintError("Can not find AudioClip:" + name);
            return null;
        }
        ...
        public AudioAsset CreateAudioAssetByPool(GameObject gameObject, bool is3D, AudioSourceType sourceType)
        {
            ...
            {
                au = new AudioAsset();
                GameObject go = new GameObject($"{sourceType}_source_{gameObject.transform.childCount}");
                go.transform.SetParent(gameObject.transform);
                go.transform.localPosition = Vector3.one;
                au.audioSource = go.AddComponent<AudioSource>();
            }
        ...
        protected void StopMusicControl(AudioAsset au, float fadeTime = 0.5f)
        {
            if (au.PlayState != AudioPlayState.Stop)
            {
                ...
                // Log.Print("StopMusicControl Stop");
                AddFade(au, VolumeFadeType.FadeOut, fadeTime, 0, null, (value) =>
                 {
                     //  Log.PrintError("StopMusicControl Stop fade CallBack");
                     value.Stop();
                 }, null);
            }
        }
        ...
        {
            // Log.PrintError("UpdateFade Count: " + fadeData.Count);
            if (fadeData.Count > 0)
            {
                foreach (var item in fadeData.Values)
                {
                    ...
                    switch (item.fadeType)
                    {
                        ...
                        case VolumeFadeType.FadeOut2In:
                            // Log.Print("FadeOut2In");
                            isComplete = FadeOut2In(item);
                            break;
                    }
                    if (isComplete)
                    {
                        // Log.Print("isComplete");
        ...
        public void AddFade(AudioAsset au, VolumeFadeType fadeType, float fadeTime, float delay, string flag, Action<AudioAsset> fadeCompleteCallBack, Action<AudioAsset> fadeOutCompleteCallBack)
        {
            ...
            // Log.Print("AddFade:"+ data.fadeType);
            //Log.Print("Add  AddFade:" + JsonUtils.ToJson(data));
        }
        ...
        private bool FadeIn(VolumeFadeData data)
        {
            ...
            //Log.Print("FadeIn:data.au.Volume " + data.au.Volume + "  dowm :" + oldVolume);
        ...
        {
            //Log.Print("FadeOut:"+ data.au.AssetName);
            if (data.au == null || data.au.audioSource == null || data.au.audioSource.clip == null)
            {
                data.au.Volume = 0;
                data.tempVolume = 0;
            ...
            oldVolume = oldVolume - speed * Time.unscaledDeltaTime;
            //Log.Print("FadeOut:data.au.Volume " + data.au.Volume + "  dowm :" + oldVolume);
            //Log.Print(" FadeOut fade State :" + data.fadeState);
            data.au.Volume = oldVolume;
            data.tempVolume = oldVolume;
        ...
        {
            //Log.Print("FadeOut2In:data.au.Volume " + data.au.Volume + "  dowm :" + data.tempVolume+ " data.fadeState:"+ data.fadeState+ "  data.fadeType：" + data.fadeType+" :"+JsonUtils.ToJson(data));
            ...
            else if (data.fadeState == VolumeFadeStateType.Delay)
            {
                ...
                //Log.Print("Delay : data.delayTime:" + data.delayTime + "  Time.unscaledDeltaTime:" + Time.unscaledDeltaTime);
                if (data.delayTime <= 0)
                {
                    data.fadeState = VolumeFadeStateType.FadeIn;
                    return false;
